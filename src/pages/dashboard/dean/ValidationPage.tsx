...
const ValidationPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      if (!decoded?.id) {
        setError(t("dean.dashboard.validation_page.messages.session_expired"));
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        setError(t("dean.dashboard.validation_page.messages.error_load"));
      } finally {
        setLoading(false);
      }
    };
  ...
  const getStatusConfig = (status: SupportCours['statut']) => {
    switch (status) {
      case 'VALIDÉ': return { label: t("dean.dashboard.validation_page.status.approved"), color: 'bg-emerald-50 text-emerald-600 border-emerald-200', icon: CheckCircle2 };
      case 'REJETÉ': return { label: t("dean.dashboard.validation_page.status.rejected"), color: 'bg-rose-50 text-rose-600 border-rose-200', icon: AlertCircle };
      case 'SOUMIS': return { label: t("dean.dashboard.validation_page.status.pending"), color: 'bg-amber-50 text-amber-600 border-amber-200', icon: Clock };
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1">
          <h1 className="text-3xl font-bold tracking-tight">{t("dean.dashboard.validation_page.title")}</h1>
          <p className="text-muted-foreground">
            {t("dean.dashboard.validation_page.department_label")} <span className="text-foreground font-semibold">{deanDepartment || t("common.loading")}</span>
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <div className="relative w-full sm:w-80 group">
            ...
            <Input 
              placeholder={t("dean.dashboard.validation_page.search_placeholder")} 
      ...
      <div className="flex items-center gap-2 p-1 bg-muted/30 rounded-xl w-fit border border-border/50">
        {(['TOUT', 'SOUMIS', 'VALIDÉ', 'REJETÉ'] as const).map((status) => (
          ...
          >
            {status === 'TOUT' ? t("dean.dashboard.validation_page.filters.all") : 
             status === 'SOUMIS' ? t("dean.dashboard.validation_page.filters.submitted") :
             status === 'VALIDÉ' ? t("dean.dashboard.validation_page.filters.validated") :
             t("dean.dashboard.validation_page.filters.rejected")}
          </button>
        ))}
      </div>
      ...
      {loading ? (
                    ...
                    <div className="relative aspect-[4/5] overflow-hidden bg-muted cursor-pointer" onClick={() => navigate(`/dashboard/dean/support/view/${item.id}`)}>
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center p-6">
                        <Button variant="secondary" className="rounded-lg font-bold gap-2">
                          {t("dean.dashboard.validation_page.actions.examine")} <ArrowRight size={16} />
                        </Button>
                      </div>
                    ...
                    <div className="p-5 space-y-3">
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-border/50">
                        <div className="flex items-center gap-2 text-xs font-semibold text-muted-foreground">
                          ...
                          {new Date(item.dateDepot).toLocaleDateString(i18n.language === 'fr' ? 'fr-FR' : 'en-US')}
                        </div>
        ...
        <div className="py-24 text-center border-2 border-dashed border-border/50 rounded-2xl bg-muted/5">
          ...
          <h3 className="text-xl font-bold mb-1">{t("dean.dashboard.validation_page.empty_title")}</h3>
          <p className="text-muted-foreground max-w-xs mx-auto text-sm">{t("dean.dashboard.validation_page.empty_desc")}</p>
        </div>
      )}
    </div>
  );
