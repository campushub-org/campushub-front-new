...
const getStatusConfig = (statut: string) => {
  switch (statut) {
    case 'VALIDÉ': return { icon: CheckCircle2, color: 'text-emerald-500', bg: 'bg-emerald-500/10', label: 'Validé' };
    case 'REJETÉ': return { icon: XCircle, color: 'text-rose-500', bg: 'bg-rose-500/10', label: 'Rejeté' };
    case 'SOUMIS': return { icon: Clock, color: 'text-amber-500', bg: 'bg-amber-500/10', label: 'À Valider' };
    default: return { icon: Bell, color: 'text-blue-500', bg: 'bg-blue-500/10', label: 'Système' };
  }
};
    ...
    <div className="max-w-6xl mx-auto py-6 px-4 animate-in fade-in duration-500">
      ...
      <div className="flex items-center justify-between mb-4 px-2">
        <div className="flex items-center gap-2 text-muted-foreground">
          <Building size={18} />
          <h1 className="text-sm font-bold uppercase tracking-widest">Dossiers de Faculté</h1>
          <Badge variant="secondary" className="ml-2 rounded-md font-bold px-1.5 py-0">
            {notifications.filter(n => !n.isRead).length} Urgent
          </Badge>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-border rounded-2xl shadow-soft overflow-hidden">
        {loading ? (
          <div className="p-20 flex flex-col items-center justify-center gap-4">
             ...
             <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Récupération des dossiers...</span>
          </div>
                      ...
                      <div className="flex-1 min-w-0 grid grid-cols-1 md:grid-cols-12 items-center gap-4">
                        <div className="md:col-span-4 flex flex-col min-w-0">
                          ...
                          <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                            <Clock size={10} /> {new Date(notif.createdAt).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })}
                          </span>
                        </div>

                        <div className="md:col-span-5 flex items-center gap-2 overflow-hidden">
                          <Badge variant="outline" className="text-[10px] font-bold bg-muted/50 border-none px-2 py-0 h-5 shrink-0 text-primary">
                            {notif.filiere}
                          </Badge>
                          ...
                          <div className="p-6 ml-14 grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            <div className="space-y-4">
                              <div className="flex items-start gap-3">
                                ...
                                <div className="space-y-1">
                                  <h5 className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Description du dossier</h5>
                                  <p className="text-sm text-foreground/80 leading-relaxed font-medium">
                                    {notif.description || "Aucun résumé descriptif n'est disponible pour ce support."}
                                  </p>
                                </div>
                              </div>
                              <Button variant="outline" size="sm" className="rounded-xl font-bold h-9 px-4" onClick={() => window.location.href='/dashboard/dean/validations'}>
                                <FileText className="mr-2 h-4 w-4 text-primary" /> Ouvrir le dossier de validation
                              </Button>
                            </div>
                            ...
                            <div className="space-y-4">
                               <div className="flex items-start gap-3 p-4 rounded-2xl bg-white dark:bg-slate-800 border border-border/50 shadow-sm">
                                  ...
                                  <div className="space-y-1">
                                    <h5 className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Historique des décisions</h5>
                                    <p className="text-sm font-medium italic">
                                      {notif.remarqueDoyen ? `"${notif.remarqueDoyen}"` : "Aucune décision ou remarque n'a encore été enregistrée."}
                                    </p>
                                  </div>
                               </div>
                            </div>
          ...
          <div className="p-32 text-center flex flex-col items-center gap-4">
             ...
             <div>
                <h3 className="text-lg font-bold">Flux de travail à jour</h3>
                <p className="text-xs text-muted-foreground mt-1 max-w-xs">Toutes les soumissions de votre faculté ont été traitées.</p>
             </div>
          </div>
        )}
      </div>
