...
const TeacherSupportPage: React.FC = () => {
  ...
  const fetchMaterials = async () => {
    ...
    try {
      ...
      toast.error(t('teacher.support.messages.error_load'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAction = async (id: number, action: 'submit' | 'delete') => {
    try {
      if (action === 'submit') {
        ...
        toast.success(t('teacher.support.messages.success_submit'));
      } else {
        await api.delete(`/campushub-support-service/api/supports/${id}`);
        setMaterials(prev => prev.filter(m => m.id !== id));
        toast.success(t('teacher.support.messages.success_delete'));
      }
    } catch (err) {
      toast.error(t('teacher.support.messages.error_action'));
    }
  };

  const getStatusConfig = (status: Material['statut']) => {
    switch (status) {
      case 'VALIDÉ': return { label: t('teacher.support.statuses.validated'), color: 'bg-emerald-500/10 text-emerald-600 border-emerald-200', icon: CheckCircle2 };
      case 'REJETÉ': return { label: t('teacher.support.statuses.rejected'), color: 'bg-rose-500/10 text-rose-600 border-rose-200', icon: XCircle };
      case 'SOUMIS': return { label: t('teacher.support.statuses.pending'), color: 'bg-amber-500/10 text-amber-600 border-amber-200', icon: Clock };
      default: return { label: t('teacher.support.statuses.draft'), color: 'bg-slate-100 text-slate-600 border-slate-200', icon: FileText };
    }
  };
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1">
          <h1 className="text-3xl font-bold tracking-tight">{t('teacher.support.title')}</h1>
          <p className="text-muted-foreground">
            {t('teacher.support.description')}
          </p>
        </div>
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <div className="relative w-full sm:w-80 group">
            ...
            <Input 
              placeholder={t('teacher.support.search_placeholder')} 
          ...
          >
            <Plus size={18} /> {t('teacher.support.new_button')}
          </Button>
        </div>
      </div>
      ...
      {loading ? (
                    ...
                    <div className="relative aspect-[4/5] overflow-hidden bg-muted cursor-pointer" onClick={() => navigate(`/dashboard/teacher/support/view/${item.id}`)}>
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center p-6">
                        <Button variant="secondary" className="rounded-lg font-bold gap-2">
                          {t('teacher.support.view_details')} <ArrowUpRight size={16} />
                        </Button>
                      </div>
                    ...
                    <div className="p-5 space-y-3">
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-border/50">
                        <div className="flex items-center gap-2 text-xs font-bold text-muted-foreground">
                          <GraduationCap size={14} className="text-primary" /> {t('teacher.dashboard.widgets.materials.table.course')} {item.niveau}
                        </div>
                        ...
                        <div className="flex items-center gap-1">
                          {item.statut === 'BROUILLON' && (
                            <Button 
                              ...
                              title={t('teacher.support.actions.submit')}
                            >
                              <Send size={16} />
                            </Button>
                          )}
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-48 rounded-xl p-2">
                              <DropdownMenuItem className="rounded-lg gap-2 cursor-pointer" onClick={() => navigate(`/dashboard/teacher/support/view/${item.id}`)}>
                                <Eye size={16} /> {t('teacher.support.actions.details')}
                              </DropdownMenuItem>
                              {item.statut === 'BROUILLON' && (
                                <AlertDialog>
                                  <AlertDialogTrigger asChild>
                                    <DropdownMenuItem onSelect={(e) => e.preventDefault()} className="rounded-lg gap-2 cursor-pointer text-destructive focus:text-destructive">
                                      <Trash2 size={16} /> {t('teacher.support.actions.delete')}
                                    </DropdownMenuItem>
                                  </AlertDialogTrigger>
                                  <AlertDialogContent>
                                    <AlertDialogHeader>
                                      <AlertDialogTitle>{t('teacher.support.dialogs.delete_title')}</AlertDialogTitle>
                                      <AlertDialogDescription>{t('teacher.support.dialogs.delete_desc')}</AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter>
                                      <AlertDialogCancel className="rounded-xl font-bold">{t('teacher.support.dialogs.cancel')}</AlertDialogCancel>
                                      <AlertDialogAction onClick={() => handleAction(item.id, 'delete')} className="bg-destructive text-white rounded-xl font-bold">{t('teacher.support.dialogs.confirm_delete')}</AlertDialogAction>
                                    </AlertDialogFooter>
                                  </AlertDialogContent>
                                </AlertDialog>
                              )}
        ...
        <div className="py-32 text-center border-4 border-dashed border-border/50 rounded-[48px] bg-muted/5">
          ...
          <h3 className="text-2xl font-bold mb-2">{t('teacher.support.empty_title')}</h3>
          <p className="text-muted-foreground max-w-sm mx-auto mb-8">{t('teacher.support.empty_desc')}</p>
          <Button onClick={() => setIsDepositDrawerOpen(true)} size="lg" className="rounded-xl font-bold px-8 shadow-glow">
            <Plus className="mr-2 h-5 w-5" /> {t('teacher.support.empty_button')}
          </Button>
        </div>
      )}
