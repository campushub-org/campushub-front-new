...
const StudentNotificationsPage: React.FC = () => {
  ...
  return (
    <TooltipProvider>
      <style>{`
        main:has(#student-notifications-area) { padding: 0 !important; overflow: hidden !important; }
        div:has(> #student-notifications-area).max-w-7xl { max-width: none !important; margin: 0 !important; width: 100% !important; }
      `}</style>

      <div id="student-notifications-area" className="flex flex-col h-[calc(100vh-4rem)] w-full bg-background overflow-hidden border-t border-border/50">
        ...
        <div className="px-6 py-8 space-y-6 shrink-0 bg-background">
          ...
          <div className="flex items-center gap-2 p-1 bg-muted/30 rounded-xl w-fit border border-border/40">
            ...
            <button
              ...
              className={cn(
                "px-5 py-1.5 rounded-lg text-xs font-bold transition-all duration-200 relative",
                filter === 'UNREAD' 
                  ? "bg-background text-primary shadow-sm border border-border/50" 
                  : "text-muted-foreground hover:text-foreground"
              )}
        ...
        {/* List Area - Modèle Doyen (max-w-4xl pl-2) */}
